...
export const metadata = { title: "工程实践 · LLM Notes" };

export default function EngineeringPage() {
  const posts = getAllPosts("engineering");

  return (
    <div className="max-w-3xl mx-auto px-6 py-16">
      <div className="mb-10">
        <h1 className="text-3xl font-semibold tracking-tight">工程实践</h1>
        <p className="text-ink-500 mt-3 text-sm">
          训练、推理、Agent 框架、Infra 的实践记录。
        </p>
      </div>
      ...
      {posts.length === 0 ? (
        ...
        <ul className="space-y-7 border-t border-[#ececec] pt-8">
          {posts.map((p) => (
            <li key={p.slug}>
              <Link href={`/engineering/${p.slug}`} className="block group">
